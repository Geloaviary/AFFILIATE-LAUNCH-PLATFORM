...
try {
  ...
  const appUrl =
    process.env.APP_URL ||
    "https://affiliate-launch-platform.vercel.app";

  const response =
    await fetch(
      `${appUrl}/api/video?action=process-render-jobs`
    );
  ...
  console.error(
    "FAILED TO TRIGGER RENDER WORKER",
    e
  );
